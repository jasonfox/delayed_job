...
<pre><code>
  create_table :delayed_jobs, :force => true do |table|
    ...
    table.string        :job_queue                    # Name of the job queue (if using named queues)
    table.timestamps
  end
</code></pre>
...
You can now submit jobs to a specific named queue and start workers that will only work off jobs in that queue. This feature is useful if you have jobs that you'd like to segment off onto a separate server.
<pre><code>
  >> Delayed::Job.enqueue SimpleJob.new, 0, Time.now, "simple_jobs"
  >> Delayed::Worker.new(:queue => "simple_jobs").start
</code></pre>
